...
j = starting_waypoint
size = len(point)
err = nq.zeros((4,2))   #4hang 2lie shu zu
#print err
...
while not rospy.is_shutdown() and exit == 0:
    ...
    try:
        ...
        continue
#坐标变换
    wRv = tf.transformations.quaternion_matrix(rot)    #四元数到矩阵
    wTv = tf.transformations.translation_matrix(trans)
    wMv = nq.dot(wTv, wRv)                                                       #矩阵的乘法运算
    vMw = tf.transformations.inverse_matrix(wMv)
    
    #points
    wRp = tf.transformations.quaternion_matrix(point_rot)#四元数到矩阵
    #point1
    wTp1 = tf.transformations.translation_matrix(point_trans1) #通过向量来求旋转矩阵
    wMp1 = nq.dot(wTp1, wRp) #矩阵的乘法运算
    vMp1 = nq.dot(vMw, wMp1)
    #point2
    wTp2 = tf.transformations.translation_matrix(point_trans2)#通过向量来求旋转矩阵
    wMp2 = nq.dot(wTp2, wRp)
    vMp2 = nq.dot(vMw, wMp2)

    #distance error definition from the vehicle to the second point (point2)
    vTp = tf.transformations.translation_from_matrix(vMp2)           #通过旋转矩阵来求向量
